<div x-data="{ open: false }">
    ...
    <aside :class="open ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 transition-transform duration-300 transform bg-white border-r border-gray-200 lg:translate-x-0">
        <div class="flex flex-col h-full">
            ...
            <nav class="flex-1 px-0 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('colocations.index') }}" class="{{ $base }} {{ request()->routeIs('colocations.index') ? $active : $inactive }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                    {{ __('My Colocation') }}
                </a>

                <a href="{{ route('colocations.historique') }}" class="{{ $base }} {{ request()->routeIs('colocations.historique') ? $active : $inactive }}">
            ...
            <div class="p-4 border-t border-gray-100">
                ...
                <a href="{{ route('profile.edit') }}" class="block px-2 py-2 text-xs font-medium text-gray-600 hover:text-gray-900">Profile</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-2 py-2 text-xs font-medium text-red-600 hover:text-red-700">Log Out</button>
                </form>
            </div>
        </div>
    </aside>
