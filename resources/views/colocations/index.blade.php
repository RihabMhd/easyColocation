...
<div class="space-y-6">
    <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <button onclick="openModal('createModal')" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg shadow hover:bg-indigo-500">
            + New Colocation
        </button>
    </header>

    @if($colocations->isEmpty())
        <div class="flex flex-col items-center justify-center p-12 bg-white border border-gray-200 border-dashed rounded-xl text-center">
            ...
            <button onclick="openModal('createModal')" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg">Create first colocation</button>
        </div>
    @else
        <div class="grid gap-6 md:grid-cols-2">
            @foreach($colocations as $colocation)
                <div class="relative bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:border-indigo-500 transition-all">
                    <div class="flex items-start justify-between mb-4">
                        <h3 class="text-lg font-bold text-gray-950">{{ $colocation->name }}</h3>
                        <span class="px-2 py-1 text-xs font-medium rounded-md bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20">{{ $colocation->status }}</span>
                    </div>
                    <p class="text-sm text-gray-600 line-clamp-2 mb-6">{{ $colocation->description }}</p>
                    
                    <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                        <a href="{{ route('colocations.show', $colocation->id) }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500">View</a>
                        <button onclick="openEditModal({{ $colocation->id }}, '{{ addslashes($colocation->name) }}', '{{ addslashes($colocation->description) }}')" class="text-sm font-semibold text-gray-600">Edit</button>
                        <div class="ml-auto text-xs text-gray-400">👥 {{ $colocation->memberships->count() }} members</div>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <form action="{{ route('colocations.store') }}" method="POST" class="p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
                </div>
            </div>
            <div class="mt-8 flex justify-end gap-3">
                <button type="button" onclick="closeModal('createModal')" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-50 rounded-lg hover:bg-gray-100">Cancel</button>
